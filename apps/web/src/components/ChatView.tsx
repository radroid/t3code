...
export default function ChatView(props: ChatViewProps) {
  ...
  // Thread outbox ("queue"): submitting while the thread is disconnected, or
  // while earlier messages are still waiting, enqueues instead of erroring, and
  // the drain sends the head once the environment is back. A running turn is
  // not a queue condition here: upstream's client-side queue (#11673) holds a
  // mid-turn send until the next tool boundary and steers it in from there.
  ...
  // One breadcrumb per submit that actually dispatches: whether it queued to the
  // outbox or went out, plus every input to that decision. It is fired at each
  // path's real dispatch point rather than at the top of `onSend`, where it also
  // logged submits the guards had already rejected (radroid/t3code#40 A3).
