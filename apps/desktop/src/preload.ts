...
contextBridge.exposeInMainWorld("desktopBridge", {
  ...
  coilUpdate: {
    getState: () => ipcRenderer.invoke(IpcChannels.COIL_UPDATE_GET_STATE_CHANNEL),
    onState: (listener) => {
      const wrappedListener = (_event: Electron.IpcRendererEvent, state: unknown) => {
      ...
      ipcRenderer.on(IpcChannels.COIL_UPDATE_STATE_CHANNEL, wrappedListener);
      return () => {
        ipcRenderer.removeListener(IpcChannels.COIL_UPDATE_STATE_CHANNEL, wrappedListener);
      };
    },
    restartNow: () => ipcRenderer.invoke(IpcChannels.COIL_UPDATE_RESTART_CHANNEL),
    dismiss: (shortSha) => ipcRenderer.invoke(IpcChannels.COIL_UPDATE_DISMISS_CHANNEL, shortSha),
  },
