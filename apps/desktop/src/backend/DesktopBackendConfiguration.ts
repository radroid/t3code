...
> {
  ...
  const command =
    runtime.kind === "executable"
      ? [runtime.entryPath]
      : // Match the primary backend's heap headroom. A node-script runtime is plain
        // node, so the flag is a V8 execArg (before the entry script) rather than
        // NODE_OPTIONS. The self-contained executable owns its own V8 flags.
        [runtime.nodePath, DESKTOP_BACKEND_MAX_OLD_SPACE_FLAG, runtime.linuxEntryPath];
