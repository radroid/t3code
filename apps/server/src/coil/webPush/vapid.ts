...
export class WebPushVapid extends Context.Service<WebPushVapid, WebPushVapidKeys>()(
  "t3/coil/webPush/vapid/WebPushVapid",
) {}
  ...
  Effect.gen(function* () {
    ...
    yield* secrets
      ...
      .pipe(
        Effect.catch((cause) =>
          Effect.logWarning(
            "coil web-push: failed to persist VAPID keys; using an in-memory keypair for this session (existing browser subscriptions will need to re-register after restart)",
            { cause },
          ),
        ),
      );
