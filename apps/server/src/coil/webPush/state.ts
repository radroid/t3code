 ...
 * coil/autoResume/state.ts.
 *
 * @module coil/webPush/state
 */
...
>()("t3/coil/webPush/state/PushSubscriptionStore") {}
  ...
  Effect.gen(function* () {
    ...
    if (fileExists) {
      ...
      if (contents === null) {
        ...
        yield* Effect.logWarning(
          "coil web-push: state file present but unreadable; running in-memory only this session so the existing file is not overwritten",
          { stateFilePath },
        );
      } else {
    ...
    const persist = (state: WebPushState): Effect.Effect<void> => {
      ...
      return writeFileStringAtomically({
        ...
        Effect.catch((cause) =>
          Effect.logWarning("coil web-push: failed to persist state", { stateFilePath, cause }),
        ),
      );
    };
