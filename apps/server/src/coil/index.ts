/**
 * coil — fork-local feature aggregator.
 *
 * Every fork-local server feature fans in here so that `apps/server/src/server.ts` needs
 * exactly ONE import and ONE `Layer.provideMerge(CoilLayerLive)` — no matter how many
 * features are added. This bounds the fork's conflict surface against upstream to a
 * single 2-line seam (see docs/coil/SEAMS.md and the design specs under
 * docs/superpowers/specs/).
 *
 * To add a feature: build it under `apps/server/src/coil/<feature>/`, then merge its
 * self-starting layer into `CoilLayerLive` below. Do NOT add a new edit to any
 * upstream-owned file.
 *
 * @module coil
 */
 ...
 * both `CoilLayerLive` and `CoilRoutesLive` reference the SAME layer value — Effect memoises
...
/**
 * All fork-local HTTP routes, fanned in here for the same reason as `CoilLayerLive`:
 ...
 *  2. **Still one instance.** Both this layer and `CoilLayerLive` provide the *same*
