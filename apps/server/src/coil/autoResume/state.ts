...
export class AutoResumeStore extends Context.Service<AutoResumeStore, AutoResumeStoreShape>()(
  "t3/coil/autoResume/state/AutoResumeStore",
) {}
  ...
  Effect.gen(function* () {
    ...
    if (fileExists) {
      ...
      if (contents === null) {
        ...
        yield* Effect.logWarning(
          "coil auto-resume: state file present but unreadable; running in-memory only this session so the existing file is not overwritten",
          { stateFilePath },
        );
      } else {
    ...
    const persist = (state: AutoResumeState): Effect.Effect<void> => {
      ...
      return writeFileStringAtomically({
        ...
        Effect.catch((cause) =>
          Effect.logWarning("coil auto-resume: failed to persist state", {
            stateFilePath,
            cause,
          }),
        ),
