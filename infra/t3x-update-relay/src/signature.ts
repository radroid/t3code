...
export function describeSignatureFailure(failure: SignatureFailure): string {
  switch (failure.kind) {
    case "missing-signature":
      return "Missing X-T3X-Signature header.";
    case "missing-timestamp":
      return "Missing X-T3X-Timestamp header.";
    case "malformed-signature":
      return "X-T3X-Signature must be of the form sha256=<hex>.";
    case "malformed-timestamp":
      return `X-T3X-Timestamp must be integer epoch seconds, received "${failure.value}".`;
    case "timestamp-skew":
      return `X-T3X-Timestamp is ${failure.skewSeconds}s away from now, which exceeds the ${MAX_TIMESTAMP_SKEW_SECONDS}s window.`;
    case "mismatch":
