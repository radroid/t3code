...
**Fork:** `radroid/t3code`
**Builds on:** `2026-07-23-fork-upstream-sync-design.md` (the daily/weekly sync + `t3x-sync` escalation contract)

## Problem

The daily upstream sync (`t3x-upstream-sync.yml`) rebases the fork onto `pingdotgg/t3code`
and, when it hits a conflict / red verify / dropped patch, is supposed to open a `t3x-sync`
...
- Replacing the daily/weekly workflows or the `t3x-sync` escalation contract. This rides on top.
...
daily sync → conflict/verify-fail/dropped-patch
   → opens t3x-sync issue (status JSON + "▶ To fix: comment @claude resolve")
   → user comments  @claude resolve   (or runs the workflow manually)
   → t3x-sync-resolve.yml runs Claude Code in CI:
...
- **Gating (`if:`):** runs for a manual dispatch, **or** an `@claude resolve` comment on an
  issue (not a PR) that carries the `t3x-sync` label **and** whose commenter is
...
- **The trigger is the primary gate.** Only a manual dispatch, or an `@claude resolve` comment
  from an `OWNER`/`MEMBER`/`COLLABORATOR` on a `t3x-sync`-labelled issue, runs the job. In agent
...
- **`main` protection is by convention, not structure.** The job needs `contents: write` to push
  ...
  push only `t3x/sync-*` + open a PR, and the **human PR-review gate** is the real backstop; if
...
auto-responder workflow, **Skip** it — this repo ships its own `t3x-sync-resolve.yml`, and a
...
- Delete/disable `t3x-sync-resolve.yml` (`gh workflow disable`). The daily sync still escalates
  the `t3x-sync` issue; the user resolves locally via the runbook's fallback steps.
