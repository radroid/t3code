...
| Loops on the page | Roadmap section, tagged `planned` | Loops lead the _direction_, not the feature list. Nothing loop-related has shipped: `docs/t3x/loop/` is design-only, archived in `67fb23e81` when its branch was deleted.                                                                    |
