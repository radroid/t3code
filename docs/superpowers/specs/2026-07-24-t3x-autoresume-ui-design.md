...
- Everything else is **new files** under `apps/server/src/t3x/` and a new `apps/web/src/t3x/`.
...
- **New file `apps/server/src/t3x/autoResume/http.ts`** exposing a raw `HttpRouter` with:
  - `GET  /api/t3x/auto-resume?threadId=…` → `{ enabled, overridePrompt, pending: { resumeAtMs, reason } | null }`
  - `POST /api/t3x/auto-resume` body `{ threadId, enabled?, overridePrompt? }` → writes to `AutoResumeStore`.
...
- **Sharing the store:** the route handler and the reactor must use the **same** `AutoResumeStore` instance. Wire `AutoResumeStoreLive` so it's visible to both the routes layer and the reactor (exploration flagged flipping `provide`→`provideMerge` for the store in `apps/server/src/t3x/index.ts` — confirm the layer graph and make the store a shared dependency, not two instances).

### C. The control UI — new `apps/web/src/t3x/`

- **New component** `apps/web/src/t3x/AutoResumeOverlay.tsx` — a floating/absolute-positioned card. Reuses existing primitives: `ui/switch` (toggle) and `ui/textarea` (resume message). Shows the live status line.
...
- **`apps/server/src/t3x/autoResume/state.ts`:** add `enabled` to `ThreadRecord` as **optional with default true** (`Schema.optionalWith(Schema.Boolean, { default: () => true })` or equivalent) so pre-existing state files still decode. Add a `setEnabled(threadId, enabled)` mutation and a `setOverridePrompt(threadId, text)` mutation (the store already carries `overridePrompt`; add the setter).
- **`apps/server/src/t3x/autoResume/Reactor.ts`:** gate on `enabled`. In detection (`onRuntimeEvent`) skip scheduling when the thread is disabled; in `fireOne` treat disabled as a cancel reason (clear pending, optional activity note). Simplest: check `record.enabled !== false` in detection, and re-check in `fireOne` against a fresh read.

## Data flow

```
toggle/textbox → fetch POST /api/t3x/auto-resume → AutoResumeStore.setEnabled / setOverridePrompt (durable JSON)
reactor detection/fireOne → reads enabled + overridePrompt from the same store
card status ← fetch GET /api/t3x/auto-resume ← AutoResumeStore (enabled, overridePrompt, current pending)
...
- `apps/web/src/t3x/AutoResumeOverlay.tsx` + the single mount line in the chat route file.
...
- Update `docs/t3x/SEAMS.md`: record the 2 new seams (server.ts route mount; web route-file overlay mount) and confirm contracts untouched.
...
- New code lives under `apps/server/src/t3x/` and `apps/web/src/t3x/` only.
