...
Cloning is Git's job, not the API's: a private Bitbucket repository also needs an app password or
repository access token saved in the Git credential helper on the machine running T3 Code. Cloning it
once from a terminal on that machine is the quickest way to store one.
...
- **Clone failed:** the message names the cause. An authentication failure means the machine running
  T3 Code has no stored Git credential for that host; "no repository was found" also covers a private
  repository that the stored credential cannot see. Cloning from a URL uses that machine's Git
  credentials, not the provider tokens above.
