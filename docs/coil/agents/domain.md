...
- **`docs/coil/CONTEXT.md`** — the fork's glossary. Covers `t3x` vocabulary only.
- **`docs/coil/adr/`** — read ADRs that touch the area you're about to work in.
...
`docs/coil/CONTEXT.md` should therefore stay small: it defines things like the `t3x` seam, the thread
...
`docs/coil/SEAMS.md` is a domain doc in everything but name: it records which upstream files the fork
