...
`17e34773b1` (the 2026-09-22 sync).

> **Re-baselined 2026-09-22** against `17e34773b1`, after a **66-commit** upstream range (issue
> #156). Three conflicts: `ci.yml` (retired-workflow modify/delete, kept deleted), `channels.ts`
> (upstream's recording-input channel placed above the fork's appended block, still `+10/-0`) and
> `pnpm-lock.yaml` (regenerated from the fork's lock). **Every code seam kept its exact pre-sync
> footprint.** Only `ci.yml` and the lock moved in the merge. The header also absorbs drift since
> the last re-baseline: `pnpm-workspace.yaml` was `+29` since #155 and #157, while this ledger
> still said `+23`.
>
...
>   the queue button) puts `apps/web/src` at 1232 findings against upstream's ceiling of 1207.
...
| `apps/web/src/components/chat/ChatComposer.tsx` | +16/-2 | 103 | **1854** | Threads `sendLabel` / `canQueue` through the composer **2026-09-20:** upstream removed `showSendWhileRunning` (#11673); the fork's two props are unchanged. |
...
| `apps/web/src/components/settings/SettingsSidebarNav.tsx` | +2/-0 | 32 | **64** | **Row 46, added 2026-09-02 (loops phase 4).** One icon import (`RefreshCwIcon`) and its `SETTINGS_SECTION_ICONS` entry for `/settings/loops`. Type-forced by the same `Readonly<Record<SettingsPath, …>>` that forces row 45, so the two land together or neither compiles. The smallest possible shape of a settings section, and fully additive |
