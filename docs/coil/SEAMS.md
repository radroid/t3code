...
Measured, not asserted: **78 upstream-owned files, +2688 / -5064 lines**, against merge-base
`7445aa733a` (the 2026-09-20 sync).
...
| `apps/web/src/routes/__root.tsx` | +9/-0 | 23 | **207** | Mounts `<NotificationCoordinator>`, `<ThreadOutboxDrain>`, `<PushSubscriptionManager>`, `<T3xUpdateToast>` |
...
| `apps/web/src/components/settings/SettingsSidebarNav.tsx` | +2/-0 | 34 | **68** | **Row 46, added 2026-09-02 (loops phase 4).** One icon import (`RefreshCwIcon`) and its `SETTINGS_SECTION_ICONS` entry for `/settings/loops`. Type-forced by the same `Readonly<Record<SettingsPath, …>>` that forces row 45, so the two land together or neither compiles. The smallest possible shape of a settings section, and fully additive |
...
| `apps/server/package.json` | +2/-0 | 31 | **62** | `web-push` dependency |
...
| `apps/desktop/src/settings/DesktopClientSettings.test.ts` | +1/-0 | 38 | **38** | `notifyOnNeedsInput` in a fixture |
...
| `third-party-licenses.config.json` | +8/-0 | 2 | **16** | **Row added 2026-09-20, and not new work.** PR #148 (2026-09-12) gave `http_ece` — the Web Push dependency — a license notice so upstream #8962's license plugin lets the web build pass. It was never registered here. |
...
| `apps/web/src/components/ThreadRouteView.tsx` | +5/-0 | 1 | **5** | **Row added 2026-09-20.** Upstream #12015 moved the thread route's body out of `routes/_chat.$environmentId.$threadId.tsx` (now `component: () => null`, identical to upstream) into this component. The fork's `<ThreadCoilOverlay>` moved with it, mounted after `{view}` inside `<SidebarInset>` for server threads — five added lines, no deletions. |
