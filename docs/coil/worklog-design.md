...
`scripts/coil/worklog/` is written against. Change the contract here first, then the code.
...
scripts/coil/worklog/
  ...
  test/run.mjs                # `node scripts/coil/worklog/test/run.mjs [name ...]`
```
...
```bash
ln -s ~/Developer/t3code/scripts/coil/worklog ~/.claude/skills/worklog
```
...
git -C ~/Developer/t3code-skills sparse-checkout set --no-cone scripts/coil/worklog
ln -s ~/Developer/t3code-skills/scripts/coil/worklog ~/.claude/skills/worklog
```
...
`node scripts/coil/worklog/test/run.mjs` (add a name to filter: `… run.mjs redact git`).
