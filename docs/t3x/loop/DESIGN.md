...
> **Status: designed, not built.** No `apps/server/src/t3x/loop/` exists — every path this
...
**Winner:** quiescence — "Deadman", shipped as **Loop Watch** (`apps/server/src/t3x/loop/`), with grafts from RUNWAY (visible mandatory budget + wall-clock deadline), Night Watch (honest terminal-state vocabulary, phone push), and BATON (scheduler-ownership line in the prompt, sticky human-cleared park).
...
# Loop Watch — `apps/server/src/t3x/loop/`

A second fork-owned reactor, shaped exactly like `apps/server/src/t3x/autoResume/`, that nudges an
...
(`apps/server/src/t3x/autoResume/classifyRateLimit.ts:56` — imported, not re-mirrored), and on a rejected
...
and `<T3xBetaSettings />` as a sibling section _inside_ the existing `<SettingsPageContainer>`, after
...
`<T3xBetaSettings />` is a fork-owned **aggregator**, not a feature component — the same discipline
`apps/server/src/t3x/index.ts` documents. The next fork settings feature hangs off it and never re-edits
the row.

Rows (all backed by the fork's own `t3x-loop.json` behind `GET/POST /api/t3x/loop`, **never**
...
- **`ThreadT3xOverlay`** (`apps/web/src/t3x/ThreadT3xOverlay.tsx`, fork-owned). The thread route's two
...
`apps/server/src/t3x/index.ts` (fork-owned, churn **0**, re-measured): a `LOOP_STATE_FILENAME`, a
module-scope `LoopStoreLive`, `LoopReactorLive.pipe(Layer.provide(Layer.mergeAll(LoopStoreLive, AutoResumeStoreLive)))`
into `T3xLayerLive`, and `loopRouteLayer.pipe(Layer.provide(LoopStoreLive))` into `T3xRoutesLive` — the
...
`apps/server/src/t3x/http/auth.ts` rather than becoming a **third** paste
...
Docs: `docs/t3x/SEAMS.md` gets the new row, two Logic-mirrors rows, one Parallel-paths row, the corrected auth-mirror note, and updated header totals — in the same commit, per SEAMS.md:24-27.

**Files:**

- `apps/server/src/t3x/http/auth.ts`
- `apps/server/src/t3x/autoResume/http.ts`
- `apps/server/src/t3x/autoResume/guards.ts`
- `apps/server/src/t3x/webPush/http.ts`
- `apps/server/src/t3x/loop/config.ts`
- `apps/server/src/t3x/loop/state.ts`
- `apps/server/src/t3x/loop/guards.ts`
- `apps/server/src/t3x/loop/sentinel.ts`
- `apps/server/src/t3x/loop/decide.ts`
- `apps/server/src/t3x/loop/Reactor.ts`
- `apps/server/src/t3x/loop/http.ts`
- `apps/server/src/t3x/loop/decide.test.ts`
- `apps/server/src/t3x/loop/guards.test.ts`
- `apps/server/src/t3x/loop/sentinel.test.ts`
- `apps/server/src/t3x/loop/state.test.ts`
- `apps/server/src/t3x/loop/http.test.ts`
- `apps/server/src/t3x/loop/Reactor.test.ts`
- `apps/server/src/t3x/loop/updatedAtContract.test.ts`
- `apps/server/src/t3x/index.ts`
- `apps/web/src/t3x/loopClient.ts`
- `apps/web/src/t3x/LoopPill.tsx`
- `apps/web/src/t3x/ThreadT3xOverlay.tsx`
- `apps/web/src/t3x/T3xBetaSettings.tsx`
- `apps/web/src/t3x/LoopBetaSection.tsx`
- `apps/web/src/routes/_chat.$environmentId.$threadId.tsx`
- `apps/web/src/components/settings/BetaSettingsPanel.tsx`
- `docs/t3x/SEAMS.md`
- `docs/t3x/loop/DESIGN.md`
...
- `apps/server/src/t3x/loop/notify.ts`
- `apps/server/src/t3x/loop/notify.test.ts`
- `apps/server/src/t3x/loop/Reactor.ts`
- `apps/server/src/t3x/loop/state.ts`
- `apps/server/src/t3x/loop/http.ts`
- `apps/web/src/t3x/LoopPill.tsx`
...
- `apps/server/src/t3x/loop/guards.ts`
- `apps/server/src/t3x/loop/guards.test.ts`
- `apps/server/src/t3x/loop/progress.ts`
- `apps/server/src/t3x/loop/progress.test.ts`
- `apps/server/src/t3x/loop/decide.ts`
- `docs/t3x/loop/DESIGN.md`

### Phase 4 — ledger paydown (independent; pays the row-35 tripwire)

Collapse the three fork mounts in `apps/web/src/routes/__root.tsx` (`NotificationCoordinator`, `ThreadOutboxDrain`, `PushSubscriptionManager` — imports at 20-22, JSX at 142-144) into one fork-owned `<T3xRoot authenticated={primaryEnvironmentAuthenticated} />` in `apps/web/src/t3x/T3xRoot.tsx`. Takes that row from 6 fork lines to 2 in a churn-9 file: **risk 54 → 18, a net −36**, and makes every future headless fork surface seam-free. The web tree then has the same two aggregators the server has had since `t3x/index.ts`.
