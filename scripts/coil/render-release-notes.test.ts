...
describe("fillCommand", () => {
  it("substitutes the bundle name and the downloaded file", () => {
    assert.strictEqual(
      ...
      'xattr -dr com.apple.quarantine "/Applications/T3 Coil (Alpha).app"',
    );
    assert.strictEqual(
      fillCommand("Unblock-File -Path .\\{file}", { app: "ignored", file: "T3Coil-abc-x64.exe" }),
      "Unblock-File -Path .\\T3Coil-abc-x64.exe",
    );
  });
...
describe("assetFileName", () => {
  // .github/workflows/coil-release.yml stages `staged/T3Coil-${SHORT_SHA}-${ARCH}.${ASSET_EXT}`.
  ...
  it("matches the names the release workflow stages", () => {
    ...
    assert.strictEqual(assetFileName(mac, "abc1234"), "T3Coil-abc1234-arm64.dmg");
    assert.strictEqual(assetFileName(win, "abc1234"), "T3Coil-abc1234-x64.exe");
  });
});
...
describe("renderReleaseNotes", () => {
  ...
  it("carries the real asset name for the sha it was given", () => {
    assert.include(render("deadbee"), "T3Coil-deadbee-x64.exe");
  });
