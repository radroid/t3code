/*
 * Renders the body of a coil GitHub release from scripts/coil/install-instructions.json.
 *
 ...
 *
 * Usage:
 *   node scripts/coil/render-release-notes.mjs --sha <full> [--short-sha <short>] [--repo-root <dir>]
 *
...
export function loadInstructions(repoRoot) {
  const file = NodePath.join(repoRoot, "scripts/coil/install-instructions.json");
...
export function desktopProductName(repoRoot) {
  ...
  return pkg.productName ?? "T3 Coil";
}
 ...
 * `staged/T3Coil-${SHORT_SHA}-${ARCH}.${ASSET_EXT}`.
 *
...
export function assetFileName(platform, shortSha) {
  ...
  return `T3Coil-${shortSha}-${arch}.${os === "darwin" ? "dmg" : "exe"}`;
}
...
export function renderReleaseNotes({ instructions, sha, shortSha }) {
  ...
  lines.push(
    "",
    "SHA-256 for every asset is in `coil-latest.json` above.",
    "",
