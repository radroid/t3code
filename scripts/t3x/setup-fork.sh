#!/usr/bin/env bash
#
# t3x fork setup — idempotent, reversible.
#
...
#
# Usage:
#   scripts/t3x/setup-fork.sh                 # configure remotes + rerere
#   scripts/t3x/setup-fork.sh --disable-workflows   # also disable inherited fork workflows (needs gh)
#   scripts/t3x/setup-fork.sh --check         # report state, change nothing
#
set -euo pipefail
...
if [[ "$DISABLE_WORKFLOWS" == 1 ]]; then
  if ! command -v gh >/dev/null; then
    ...
    warn "The two t3x workflows are intentionally left enabled."
  fi
else
