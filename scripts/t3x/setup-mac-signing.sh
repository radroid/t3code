#!/usr/bin/env bash
#
# t3x — create the macOS code-signing identity that stops the permission prompts.
#
...
#
# Usage:
#   scripts/t3x/setup-mac-signing.sh                  # create it (idempotent), then self-verify
#   scripts/t3x/setup-mac-signing.sh --status         # report; exit 0 only if usable right now
#   scripts/t3x/setup-mac-signing.sh --unlock         # unlock the keychain (builds call this)
#   scripts/t3x/setup-mac-signing.sh --print-identity # the CSC_NAME value
#   scripts/t3x/setup-mac-signing.sh --print-requirement  # the designated requirement it produces
#   scripts/t3x/setup-mac-signing.sh --print-ci-secrets   # values for the two GitHub secrets
#   scripts/t3x/setup-mac-signing.sh --rotate         # NEW certificate: re-prompts once more
#
...
# .app gets, which is what makes docs/t3x/mac-signing/designated-requirement.txt trustworthy.
print_requirement() {
  ...
  local stub
  stub="$(mktemp -d "${TMPDIR:-/tmp}/t3x-sig-stub.XXXXXX")"
...
create_identity() {
  ...
  if [[ ! -f "$CERT_PATH" || ! -f "$KEY_PATH" ]]; then
    ...
    local config
    config="$(mktemp "${TMPDIR:-/tmp}/t3x-openssl.XXXXXX")"
...
case "$MODE" in
  ...
  rotate)
    ...
    log "Record it: scripts/t3x/setup-mac-signing.sh --print-requirement > docs/t3x/mac-signing/designated-requirement.txt"
    log "And re-set the CI secrets: scripts/t3x/setup-mac-signing.sh --print-ci-secrets"
    ;;
  ensure)
    create_identity
    ...
    if [[ -n "$REPO" ]]; then
      recorded="$REPO/docs/t3x/mac-signing/designated-requirement.txt"
    ...
    log ""
    log "Next: builds pick the identity up through CSC_NAME. scripts/t3x/auto-build-desktop.sh does"
