...
describe("normalizePrompt / promptHash", () => {
  test("trims, collapses whitespace and lowercases", () => {
    assert.equal(normalizePrompt("  Hello   \n\t WORLD \n"), "hello world");
    assert.equal(normalizePrompt("Fix\r\nthe   bug"), "fix the bug");
  });

  test("keeps only the first 400 characters", () => {
    const long = `${"a".repeat(399)}bcdef`;
    const normalized = normalizePrompt(long);
    assert.equal(normalized.length, 400);
    assert.ok(normalized.endsWith("ab"));
    ...
    assert.equal(promptHash(long), promptHash(`${long}-and-much-more`));
  });
  ...
  test("survives non-strings", () => {
    ...
    assert.equal(promptHash(null), promptHash(""));
  });

  test("hashes the normalised form with sha256", () => {
    const expected = NodeCrypto.createHash("sha256").update("hello world", "utf8").digest("hex");
    assert.equal(promptHash("  Hello   WORLD \n"), expected);
    // Pinned literal so a refactor of the helper cannot quietly move the join key.
    assert.equal(expected, "b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9");
  });

  test("matches the same-named functions in lib/t3db.mjs", async (t) => {
    if (!NodeFS.existsSync(T3DB_PATH)) {
      t.skip("lib/t3db.mjs is not present yet — re-run once it lands to verify the join key");
      return;
    }
    const t3db = await import(pathToFileURL(T3DB_PATH).href);
    const samples = [
    ...
    for (const sample of samples) {
      assert.equal(t3db.normalizePrompt(sample), normalizePrompt(sample), `normalize: ${sample}`);
      assert.equal(t3db.promptHash(sample), promptHash(sample), `hash: ${sample}`);
    }
  });
});

describe("listSessionFiles", () => {
  test("returns [] for a missing directory instead of throwing", () => {
    assert.deepEqual(listSessionFiles(NodePath.join(makeTempDir(), "nope"), { since: 0 }), []);
    assert.deepEqual(listSessionFiles(undefined, { since: 0 }), []);
    assert.deepEqual(listSessionFiles("", {}), []);
  });

  test("finds transcripts one level down and ignores everything else", () => {
    ...
    assert.equal(found[0].dir, NodePath.join(projects, "-Users-dev-demo"));
    assert.ok(found[0].file.endsWith("aaa.jsonl"));
    assert.ok(Number.isFinite(found[0].mtimeMs));
  });

  test("prefilters on mtime and sorts newest first", () => {
    const projects = makeTempDir();
...
describe("readSessionFile", () => {
  test("summarises a whole session", async () => {
    const projects = makeTempDir();
    ...
    assert.equal(session.cwd, "/Users/dev/Developer/demo");
    assert.equal(session.gitBranch, "main");
    assert.deepEqual(session.versions, ["2.1.226"]);
    assert.deepEqual(session.models, ["claude-opus-5", "claude-haiku-4-5"]);
    assert.equal(session.startedAt, `${DAY}T10:00:00.000Z`);
    assert.equal(session.endedAt, `${DAY}T10:11:00.000Z`);
    assert.equal(session.promptCount, 2);
    assert.equal(session.assistantCount, 4);
    assert.equal(session.toolUseCount, 2);
    assert.equal(session.firstPrompt, "Ship the release");
    assert.equal(session.lastPrompt, "Now write the changelog");
    assert.deepEqual(session.promptHashes, [
      promptHash("Ship the release"),
      promptHash("Now write the changelog"),
    ]);
    assert.deepEqual(session.prLinks, [
      {
        number: 66,
    ...
    assert.equal(session.eventTimes.length, 10);
    assert.deepEqual(session.eventTimes, [...session.eventTimes].sort());
    assert.deepEqual(session.turnSpans, [
      { start: `${DAY}T10:00:00.000Z`, end: `${DAY}T10:05:00.000Z` },
      { start: `${DAY}T10:10:00.000Z`, end: `${DAY}T10:11:00.000Z` },
    ]);
    assert.equal(session.sidechainOnly, false);
    assert.equal(session.excluded, null);
  });

  test("keeps only in-window records and returns null when none are", async () => {
    const projects = makeTempDir();
    ...
    assert.equal(session.firstPrompt, "today");
    assert.equal(session.eventTimes.length, 2);
    assert.deepEqual(session.turnSpans, [
    ...
    assert.equal(yesterdayOnly, null);
  });

  test("tolerates a truncated final line and records with no timestamp", async () => {
    const projects = makeTempDir();
    ...
    assert.equal(session.eventTimes.length, 2);
    assert.equal(session.endedAt, `${DAY}T12:01:00.000Z`);
  });

  test("treats wrappers and injected context as non-prompts but still as activity", async () => {
    const projects = makeTempDir();
    ...
    const session = await readSessionFile(file, WINDOW);
    assert.equal(session.promptCount, 1);
    assert.equal(session.firstPrompt, "Actually fix the flake");
    assert.equal(session.lastPrompt, "Actually fix the flake");
    assert.deepEqual(session.promptHashes, [promptHash("Actually fix the flake")]);
    // Their timestamps are still real activity, and each is still a turn boundary.
    assert.equal(session.eventTimes.length, 8);
    assert.equal(session.turnSpans.length, 6);
    assert.deepEqual(session.turnSpans.at(-1), {
      start: `${DAY}T08:04:00.000Z`,
      end: `${DAY}T08:05:00.000Z`,
    });
  });

  test("ignores tool results and sidechain turns when counting prompts", async () => {
    const projects = makeTempDir();
    ...
    const session = await readSessionFile(file, WINDOW);
    assert.equal(session.promptCount, 1);
    assert.deepEqual(session.promptHashes, [promptHash("main thread prompt")]);
    assert.equal(session.sidechainOnly, false);
  ...
  test("flags a transcript that is entirely sidechain", async () => {
    const projects = makeTempDir();
    ...
    const session = await readSessionFile(file, WINDOW);
    assert.equal(session.sidechainOnly, true);
    assert.equal(session.promptCount, 0);
    assert.equal(session.firstPrompt, null);
    assert.deepEqual(session.turnSpans, []);
  });

  test("falls back to the last-prompt record when the window holds no narratable prompt", async () => {
    const projects = makeTempDir();
    ...
    assert.equal(wide.lastPrompt, "the original ask");
    ...
    assert.equal(tail.promptCount, 0);
    assert.equal(tail.firstPrompt, null);
    assert.equal(tail.lastPrompt, "the original ask");
  });

  test("truncates prompt text to 2000 characters", async () => {
    const projects = makeTempDir();
    ...
    assert.deepEqual(session.promptHashes, [promptHash("z".repeat(5000))]);
  });

  test("takes the most frequent cwd and branch when a session moves", async () => {
    const projects = makeTempDir();
    ...
    assert.equal(session.gitBranch, "new");
    assert.deepEqual(session.versions, ["2.1.226", "2.1.300"]);
  });

  test("returns null for a missing file rather than throwing", async () => {
    const missing = NodePath.join(makeTempDir(), "gone.jsonl");
    assert.equal(await readSessionFile(missing, WINDOW), null);
  });

  test("returns null for an empty transcript", async () => {
    const projects = makeTempDir();
    const file = writeTranscript(projects, "p", "s-empty", []);
    assert.equal(await readSessionFile(file, WINDOW), null);
  });

  test("defaults to an unbounded window", async () => {
    const projects = makeTempDir();
...
describe("scanSessions", () => {
  test("collects in-window sessions and sorts them by start time", async () => {
    const projects = makeTempDir();
  ...
  test("skips an entirely malformed transcript with a warning", async () => {
    ...
    assert.equal(warnings.length, 1);
    assert.match(warnings[0], /corrupt\.jsonl/u);
    assert.match(warnings[0], /failed to parse/u);
  });

  test("keeps a partly-corrupt transcript but warns about the dropped lines", async () => {
    const projects = makeTempDir();
    ...
    assert.equal(warnings.length, 1);
    assert.match(warnings[0], /Ignored 2 unparsable lines/u);
  });

  test("warns rather than throwing when a transcript cannot be opened", async (t) => {
    if (process.getuid?.() === 0) {
    ...
    assert.deepEqual(sessions, []);
    assert.equal(warnings.length, 1);
    assert.match(warnings[0], /Could not read locked\.jsonl/u);
    NodeFS.chmodSync(file, 0o600);
  });

  test("caps the scan at `limit` files, newest first, and says so", async () => {
    const projects = makeTempDir();
    ...
    assert.equal(warnings.length, 1);
    assert.match(warnings[0], /Read only the 2 most recently modified of 4/u);
  });

  test("uses the window start as the mtime prefilter", async () => {
    const projects = makeTempDir();
...
describe("linkSessions", () => {
  ...
  test("rule 1: a cwd under a worktree root is t3code-driven", () => {
  ...
  test("rule 1 matches a worktree root exactly and does not match a sibling prefix", () => {
    ...
    assert.equal(exact.excluded.rule, "worktree");
    assert.equal(sibling.excluded, null);
  });

  test("rule 2: a shared prompt hash links a session to its thread", () => {
    ...
    assert.deepEqual(target.excluded, {
      reason: "t3code-driven",
      rule: "prompt-hash",
      linkedTo: "thread-xyz",
    });
  ...
  test("rule 3: anything else stands alone", () => {
    ...
    assert.equal(standalone.excluded, null);
  });

  test("the worktree rule wins over a matching prompt hash", () => {
    ...
    assert.equal(target.excluded.rule, "worktree");
    assert.equal(target.excluded.linkedTo, "thread-abc");
  });

  test("returns the same array and tolerates thin or absent inputs", () => {
    const target = session({ cwd: null });
    const list = [target, null];
    assert.equal(linkSessions(list), list);
    assert.equal(target.excluded, null);
    assert.deepEqual(linkSessions(undefined), []);
    ...
    assert.deepEqual(snake.excluded, {
      reason: "t3code-driven",
      rule: "worktree",
      linkedTo: "t-9",
    });
    ...
    assert.equal(viaObject.excluded.linkedTo, "t-obj");
  });
});

describe("isMachineSession", () => {
  ...
  test("the prefix list is the pinned set of verified machine senders", () => {
    ...
    assert.ok(Object.isFrozen(MACHINE_PROMPT_PREFIXES));
    for (const prefix of MACHINE_PROMPT_PREFIXES) {
      ...
      assert.equal(prefix, prefix.toLowerCase(), `prefix must be stored lowercased: ${prefix}`);
      assert.equal(prefix, prefix.trim(), `prefix must not carry padding: ${prefix}`);
    }
  });

  test("machine-directive catches a system prompt nobody has listed yet", () => {
  ...
  test("machine-directive does not swallow a person who happens to open with 'You'", () => {
    ...
    for (const prompt of [
      ...
      assert.notEqual(verdict.rule, "machine-directive", `false positive on: ${prompt}`);
    }
  });

  for (const prefix of MACHINE_PROMPT_PREFIXES) {
    test(`known-prompt: a session opening with "${prefix}" is machine-generated`, () => {
      ...
      assert.deepEqual(isMachineSession(machineCandidate()), { machine: false, rule: null });

      const expected = { machine: true, rule: "known-prompt" };
      assert.deepEqual(isMachineSession(machineCandidate({ firstPrompt: prefix })), expected);

      // Prefix-based: the real prompts continue for several more paragraphs after the opening.
      const continued = `${prefix}. ${PROMPT_TAIL}`;
      assert.ok(continued.length > prefix.length + 100);
      assert.deepEqual(isMachineSession(machineCandidate({ firstPrompt: continued })), expected);
  ...
  test("utility-shape: a one-prompt, one-tool, 7-second session with an unknown prompt", () => {
    ...
    assert.deepEqual(isMachineSession(session), { machine: true, rule: "utility-shape" });
  ...
  test("utility-shape stops one millisecond short of two minutes", () => {
    ...
    assert.deepEqual(utility(0), { machine: true, rule: "utility-shape" });
    assert.deepEqual(utility(119_999), { machine: true, rule: "utility-shape" });
    assert.deepEqual(utility(120_000), { machine: false, rule: null });
    assert.deepEqual(utility(120_001), { machine: false, rule: null });
  });
  ...
  test("a short session with two prompts is a person, not a utility", () => {
    const session = machineCandidate({
    ...
    assert.deepEqual(isMachineSession(session), { machine: false, rule: null });
    linkSessions([session]);
    assert.equal(session.excluded, null);
  });

  test("a one-prompt session with five tool uses is a person, not a utility", () => {
    const session = machineCandidate({
  ...
  test("a one-prompt, one-tool session lasting ten minutes is a person, not a utility", () => {
    const session = machineCandidate({
  ...
  test("a prompt that merely contains a machine prefix is not machine-generated", () => {
    const session = machineCandidate({
      firstPrompt:
  ...
  test("never throws on thin, empty or non-string input", () => {
    const cases = [
    ...
    for (const [label, input] of cases) {
      ...
      assert.deepEqual(isMachineSession(input), { machine: false, rule: null }, label);
    }

    // A known prompt is decided before the timestamps are read, so it survives having none.
    assert.deepEqual(isMachineSession({ firstPrompt: TITLE_PROMPT }), {
      machine: true,
      rule: "known-prompt",
    });
...
describe("linkSessions: the machine rung outranks the t3code rungs", () => {
  ...
  test("a known machine prompt under a worktree root reports machine-generated", () => {
    ...
    assert.deepEqual(human.excluded, {
      reason: "t3code-driven",
      rule: "worktree",
      linkedTo: "thread-abc",
    });
  ...
  test("a utility-shaped session under a worktree root reports machine-generated too", () => {
    const session = inWorktree({
      firstPrompt: BRANCH_NAME_PROMPT,
      promptCount: 1,
      assistantCount: 2,
  ...
  test("the machine rung also outranks the prompt-hash rung", () => {
    const session = {
      sessionId: "s",
      cwd: "/Users/dev/Developer/t3code",
      promptHashes: [promptHash(TITLE_PROMPT)],
...
describe("end to end over a fixture projects directory", () => {
  test("scan then link produces an auditable, deduped set", async () => {
    const projects = makeTempDir();
  ...
  test("a real-shaped day: 12 transcripts on disk, 5 of them are a person working", async () => {
    ...
    assert.equal(genuine.length, 5);
    assert.equal(sessions.length - genuine.length, 7);
    ...
    assert.equal(titleOne.cwd, worktreeA);
    assert.deepEqual(titleOne.promptHashes, [promptHash(TITLE_PROMPT)]);
    assert.deepEqual(titleOne.excluded, {
      reason: "machine-generated",
      rule: "known-prompt",
      linkedTo: null,
    });
    ...
    assert.deepEqual(asHuman.excluded, {
      reason: "t3code-driven",
      rule: "worktree",
      linkedTo: "thread-abc",
    });
    ...
    assert.equal(security.assistantCount, 4);
    assert.equal(security.toolUseCount, 3);
    assert.equal(Date.parse(security.endedAt) - Date.parse(security.startedAt), 180_000);
    assert.deepEqual(isMachineSession({ ...security, firstPrompt: "Review the queue design" }), {
      machine: false,
      rule: null,
    });
  });
